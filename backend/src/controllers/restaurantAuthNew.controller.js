...
const checkPortal = async (userId) => {
  const r = await Restaurant.findOne({ owner: userId }).select('portalEnabled isActive');
  if (!r) return null;
  if (!r.isActive) return 'deactivated';
  if (r.portalEnabled === false) return 'disabled';
  return 'ok';
};
...
exports.register = async (req, res, next) => {
  try {
    ...
    await Pending.findOneAndUpdate(
      { email },
      { name, phone, email, password: hashed, role: 'restaurant', otp, createdAt: new Date() },
...
exports.verifyEmailOtp = async (req, res, next) => {
  try {
    ...
    const pending = await Pending.findOne({ email, role: 'restaurant' });
    ...
    return successResponse(res, { token, user }, 'Email verified! Welcome to Rodofood Restaurant Panel.', 201);
  } catch (error) {
    next(error);
  }
};
...
exports.login = async (req, res, next) => {
  try {
    ...
    const query = phone ? { phone, role: 'restaurant' } : { email, role: 'restaurant' };
    const user = await User.findOne(query);
    if (!user) return errorResponse(res, 'No restaurant account found. Please register.', 404);
    ...
    if (portalStatus === 'deactivated') return errorResponse(res, 'Your restaurant has been deactivated.', 403);
    if (portalStatus === 'disabled') return errorResponse(res, 'Your restaurant portal access has been disabled by admin.', 403);
...
exports.sendLoginOtp = async (req, res, next) => {
  try {
    ...
    if (!user) return errorResponse(res, 'No restaurant account found with this email.', 404);
    if (!user.isActive) return errorResponse(res, 'Account deactivated. Please contact support.', 403);
