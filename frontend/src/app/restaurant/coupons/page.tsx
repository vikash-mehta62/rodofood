...
import { Plus, Edit2, Trash2, Tag, Loader2, X, Percent, IndianRupee, Calendar, Users } from 'lucide-react';
...
export default function RestaurantCouponsPage() {
  ...
  const [form, setForm] = useState<CouponForm>(DEF);
  const [delId, setDelId] = useState<string | null>(null);

  const { data, isLoading } = useQuery({
    queryKey: ['my-coupons'],
    queryFn: async () => (await api.get('/coupons/my')).data.data.coupons,
  });

  const saveMut = useMutation({
    mutationFn: (body: object) => editId
      ? api.put(`/coupons/my/${editId}`, body)
      : api.post('/coupons/my', body),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['my-coupons'] }); closeForm(); },
  });

  const delMut = useMutation({
    mutationFn: (id: string) => api.delete(`/coupons/my/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['my-coupons'] }); setDelId(null); },
  });
  ...
  const openEdit = (c: any) => {
    ...
    setForm({
      ...
      validFrom: c.validFrom?.split('T')[0] || DEF.validFrom,
      validUntil: c.validUntil?.split('T')[0] || DEF.validUntil,
      isActive: c.isActive,
    });
    setShowForm(true);
  };
  ...
  const set = (k: keyof CouponForm, v: string | boolean) => setForm(p => ({ ...p, [k]: v }));
  ...
  return (
    <div className="p-4 sm:p-6 min-h-screen" style={{ background: '#F1F5F9' }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-slate-900">My Coupons</h1>
          <p className="text-slate-400 text-sm font-medium mt-0.5">Create discount codes for your restaurant</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {coupons.map((c: any) => {
            ...
            return (
              <div key={c._id} className={`bg-white rounded-2xl border overflow-hidden shadow-sm hover:shadow-md transition-all ${!active ? 'opacity-60' : 'border-slate-100'}`}>
                {/* Ticket top */}
                <div className="px-5 py-4 relative" style={{ background: active ? 'linear-gradient(135deg,#FF6B35,#FF8C42)' : '#94a3b8' }}>
                  ...
                  <div className="relative z-10 flex items-center justify-between">
                    <div>
                      <p className="text-white/70 text-[10px] font-black uppercase tracking-widest mb-1">Coupon Code</p>
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-100">
          ...
          <p className="font-black text-slate-400 text-lg">No coupons yet</p>
          <p className="text-slate-300 text-sm font-medium mt-1">Create your first discount coupon</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" style={{ backdropFilter: 'blur(4px)' }}>
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl max-h-[92vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 flex-shrink-0">
              ...
              <button onClick={closeForm} className="w-8 h-8 rounded-xl bg-slate-100 hover:bg-slate-200 flex items-center justify-center">
                <X className="w-4 h-4 text-slate-600" />
              </button>
            </div>

            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
              <Field label="Coupon Code *" value={form.code} onChange={v => set('code', v.toUpperCase())} placeholder="e.g. SAVE20" />
              <Field label="Description" value={form.description} onChange={v => set('description', v)} placeholder="e.g. 20% off on all orders" />
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(['percentage', 'flat'] as const).map(t => (
                    <button key={t} type="button" onClick={() => set('discountType', t)}
                      className={`py-3 rounded-xl border-2 text-sm font-black transition-all capitalize ${form.discountType === t ? 'border-orange-400 bg-orange-50 text-orange-600' : 'border-slate-200 text-slate-400 bg-white'}`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {form.discountType === 'percentage' && (
                  <Field label="Max Discount ₹" value={form.maxDiscountAmount} onChange={v => set('maxDiscountAmount', v)} placeholder="100" type="number" />
                )}
              </div>
