...
function RestaurantPageContent() {
  ...
  const tabs = categories.length > 0 ? ['All', ...categories] : [];
  ...
  return (
    <div className="min-h-screen bg-white pb-32">
      ...
      <div className="relative h-56 sm:h-72 bg-gradient-to-br from-orange-50 to-amber-50 overflow-hidden">
        ...
        <div className={`absolute top-12 right-4 px-3 py-1 rounded-full text-xs font-black shadow-md ${restaurant.isOpen ? 'bg-green-500 text-white' : 'bg-slate-700 text-white'}`}>
      ...
      <div className="px-4 py-4 border-b border-slate-100">
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          <div className="flex items-center gap-1.5 bg-green-50 border border-green-200 rounded-xl px-3 py-1.5">
            ...
            {restaurant.totalRatings > 0 && <span className="text-green-600/60 text-xs">({restaurant.totalRatings})</span>}
          </div>
          <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5">
            <Clock className="w-4 h-4 text-orange-500" />
            <span className="font-black text-slate-700 text-sm">{restaurant.avgPrepTimeMinutes} min</span>
            <span className="text-slate-400 text-xs">prep</span>
          </div>
          <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5">
        ...
        <div className="flex items-center gap-2 mb-3">
          <span className={`text-xs font-black px-2.5 py-1 rounded-lg border ${
            restaurant.foodType === 'veg' ? 'border-green-500 text-green-700 bg-green-50' :
            ...
            {tabs.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)}
                className={`flex-shrink-0 px-4 py-3.5 text-sm font-black border-b-2 transition-colors whitespace-nowrap ${
                  activeCategory === cat ? 'border-orange-500 text-orange-500' : 'border-transparent text-slate-400 hover:text-slate-700'
                }`}>
                {cat}
                <span className="ml-1.5 text-[10px] font-bold text-slate-300">
                  {cat === 'All' ? totalItems : (menu?.[cat]?.length ?? 0)}
                </span>
              </button>
            ))}
          </div>
      ...
      <div className="px-4">
        {mLoading ? (
          ...
          <div>
            {categories.map(cat => (
              <div key={cat}>
                <div className="flex items-center justify-between py-4 border-b border-slate-50 mt-2">
                  <h2 className="font-black text-slate-900 text-base">{cat}</h2>
                  <span className="text-xs text-slate-400">{menu[cat]?.length} items</span>
                </div>
                {(menu[cat] as any[])?.map((item: any) => (
          ...
          <div>
            <div className="flex items-center justify-between py-4">
              <h2 className="font-black text-slate-900 text-base">{activeCategory}</h2>
              <span className="text-xs text-slate-400">{(menu[activeCategory] as any[])?.length} items</span>
            </div>
            {(menu[activeCategory] as any[])?.map((item: any) => (
              <MenuItemCard key={item._id} item={item} restaurantId={restaurant._id} restaurantName={restaurant.name} />
            ))}
          </div>
        )}
